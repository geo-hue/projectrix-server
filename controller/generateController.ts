...
const openaiBreaker = new CircuitBreaker(3, 60000);
 ...
 */
function getComplexitySpecificGuidance(complexity: { level: string; percentage: number }) {
 ...
 */
function validateTechnologyCategoryPair(
  technologies: string[],
  category: string
): { valid: boolean; message: string; suggestedCategory?: string; warning?: boolean } {
  ...
  const categoryTechs = categoryTechGroups[categoryLower as keyof typeof categoryTechGroups];
  ...
  const validTechs = normalizedTechs.filter(tech => 
    categoryTechs.some(validTech => 
 ...
 */
function extractTechnologiesFromResponse(
  projectData: any,
  category: string
): string[] {
...
function shouldEnhanceProject(projectData: any): boolean {
  ...
  const hasFewOutcomes =
    !projectData.learningOutcomes || projectData.learningOutcomes.length < 3;

  // Check if roles have missing details
  const hasIncompleteRoles =
    !projectData.teamStructure ||
    !projectData.teamStructure.roles ||
    projectData.teamStructure.roles.some(
      (role: any) =>
        !role.title ||
        !role.skills ||
...
Additional Features: ${
      projectData.features?.additional?.join(", ") || "Not provided"
    }
...
${
  projectData.teamStructure?.roles
    ?.map(
      (role: any) =>
        `- ${role.title || "Unnamed role"} (Skills: ${
          role.skills?.join(", ") || "None specified"
        }, Responsibilities: ${
          role.responsibilities?.join(", ") || "None specified"
        })`
    )
    .join("\n") || "Not provided"
}
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert software architect and creative project planner. Your role is to enhance user-submitted project details while maintaining the original concept and intent. Return responses as raw JSON without LaTeX formatting like \\boxed{}.",
        },
        {
          role: "user",
    ...
    try {
      ...
      const mergedData = {
        ...
        features: {
          ...
          additional:
            enhancedData.features?.additional ||
            projectData.features?.additional ||
            [],
        },
        teamStructure: {
          roles:
            enhancedData.teamStructure?.roles?.map(
              (
                role: { title: any; skills: any; responsibilities: any },
                index: string | number
              ) => {
        ...
        learningOutcomes:
          enhancedData.learningOutcomes || projectData.learningOutcomes || [],
      };

      console.log(
        "Merged enhancement data:",
...
function mergeRoles(originalRoles: any[], enhancedRoles: any[]): any[] {
  ...
  for (let i = 0; i < mergedRoles.length; i++) {
    ...
    const matchingEnhancedRole =
      enhancedRoles.find(
        (role) => role.title.toLowerCase() === originalRole.title.toLowerCase()
      ) || enhancedRoles[0];
    ...
    if (matchingEnhancedRole) {
      mergedRoles[i] = {
        ...
        skills:
          originalRole.skills?.length > 0
            ? originalRole.skills
            : matchingEnhancedRole.skills,
        responsibilities:
          originalRole.responsibilities?.length > 0
            ? originalRole.responsibilities
  ...
  if (originalRoles.length < enhancedRoles.length) {
    ...
    const originalTitles = originalRoles.map((role) =>
...
export const generateProject = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    // Validate technology and category combinations
      const validationResult = validateTechnologyCategoryPair(
        technologies,
        category
      );
      ...
      try {
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are an expert software architect and creative project planner. Your role is to generate detailed, innovative, and practical software project ideas based on user requirements. Return responses as raw JSON without LaTeX formatting like \\boxed{}.",
              },
              {
                role: "user",
                content: prompt,
        ...
        if (error.message === 'Circuit is open') {
          return next(
            new ErrorHandler(
              "We're experiencing high demand for AI project generation. Please try again in a minute.", 
              503
            )
          );
        }
        ...
        return next(
          new ErrorHandler(
            "Unable to generate project at this time. Please try again later.",
            500
          )
        );
      }
    ...
    if (actualRoleCount > exactSize) {
      ...
      console.log(`Added additional roles to reach ${exactSize} roles`);
        ...
        if (
          ...
          console.warn(
            `Team size constraint violated: Expected ${teamSize} but got ${roleCount} roles`
          );
        }
      } catch (parseError) {
        console.error(
          "Error parsing or validating OpenAI response:",
          parseError
        );
        return next(
          new ErrorHandler(
      ...
      if (projectData.teamStructure && projectData.teamStructure.roles) {
        projectData.teamStructure.roles = projectData.teamStructure.roles.map(
          (role: any) => ({
            ...role,
...
export const getGeneratedProjects = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const userId = req.user?._id;
      const projects = await GeneratedProject.find({ userId }).sort({
        createdAt: -1,
      });
...
export const generateAnother = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const originalProject = await GeneratedProject.findOne({
        _id: projectId,
        userId,
      });
      if (!originalProject) {
      ...
      const preferences = {
        ...
        projectTheme:
          "Please generate a different project than before, with a fresh concept and approach.",
      };
...
export const startProject = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { projectId } = req.params;
      if (!req.user) {
      ...
      const project = await GeneratedProject.findOne({
        _id: projectId,
        userId,
      });
      if (!project) {
...
export const getUserSavedProjects = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
           if (!req.user) {
                return next(new ErrorHandler("Authentication required", 401));
...
export const publishProject = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { projectId } = req.params;
      ...
      if (!canPublish) {
        return next(
          new ErrorHandler(
            "You have reached your publish limit. Free users can only publish 1 project. Upgrade to Pro for unlimited publishing.",
            403
          )
        );
      }
...
export const submitUserProject = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (useEnhancement) {
        ...
        const hasEnhancementsLeft = await checkEnhancementsLimit(
          user._id.toString()
        );
        if (!hasEnhancementsLeft) {
          return next(
            new ErrorHandler(
              "No enhancements left. Pro users get 8 enhancements per month, while free users get 2.",
              403
            )
          );
        }
        ...
        if (needsEnhancement) {
          ...
          console.log(
            `User's enhancements decremented. AI enhancement applied.`
          );
        } else {
          console.log(
            "✅ Project is already well-defined. Enhancement not necessary."
          );
        }
      }
      ...
      const formattedData = {
        ...
        features: {
          core:
            enhancedData.features?.core?.filter((feature: string) =>
              feature.trim()
            ) || [],
          additional:
            enhancedData.features?.additional?.filter((feature: string) =>
              feature.trim()
            ) || [],
        },
        teamStructure: {
          roles:
            enhancedData.teamStructure?.roles?.map((role: any) => ({
              title: role.title || "",
              skills: role.skills || [],
              responsibilities:
                role.responsibilities?.filter((r: string) => r.trim()) || [],
              filled: false, // Default to not filled
            })) || [],
        },
        learningOutcomes:
          enhancedData.learningOutcomes?.filter((outcome: string) =>
            outcome.trim()
          ) || [],
      ...
      console.log(
        "\n📝 User project data to be saved",
        wasEnhanced ? " (AI enhanced)" : ""
      );
...
export const editProject = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (project.isPublished) {
        return next(
          new ErrorHandler("Published projects cannot be edited", 403)
        );
      }
      ...
      if (!userCanEdit) {
        return next(
          new ErrorHandler(
            "Only Pro users can edit saved projects. Please upgrade to Pro to enable project editing.",
            403
          )
        );
      }
      ...
      const updatedProject = {
        ...
        complexity: projectData.complexity
          ? {
              ...
              percentage:
                projectData.complexity.percentage ||
        ...
        duration: projectData.duration
          ? {
              ...
              estimate:
                projectData.duration.estimate || project.duration.estimate,
            }
          : project.duration,
        ...
        features: projectData.features
          ? {
              ...
              additional:
                projectData.features.additional || project.features.additional,
            }
          : project.features,
        teamStructure: projectData.teamStructure
          ? {
              roles: projectData.teamStructure.roles.map(
          ...
          : project.teamStructure,
        learningOutcomes:
          projectData.learningOutcomes || project.learningOutcomes,
      };
