...
const processUserLimitResets = async () => {
    try {
        ...
        // This includes:
        // 1. New users who haven't had a reset yet (use createdAt + 30 days)
        // 2. Users with a nextLimitResetDate of today
        const usersToReset = await userModel_1.default.find({
            $or: [
                ...
                // New users who haven't had a reset yet but are 30+ days old
                {
        ...
        for (const user of usersToReset) {
            // Reset limits based on user's current plan
            if (user.plan === 'pro') {
    ...
    try {
        ...
        for (const subscription of expiredSubscriptions) {
            ...
            if (user.plan === 'pro') {
                ...
                user.plan = 'free';
                // Reset limits to free plan levels
                user.projectIdeasLeft = 3;
                user.collaborationRequestsLeft = 3;
                user.enhancementsLeft = 2;
                // Save user
